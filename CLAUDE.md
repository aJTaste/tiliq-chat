...
## アカウント運用（2026年8月〜8月17日、終了）

2026年8月、Claude Proのゲストパス（友人からの紹介リンク）を誤って新規のClaudeアカウントでredeemしてしまったことをきっかけに、メイン/サブの2アカウント体制で開発を進めていた期間があった（メインが利用制限に達した場合のみサブに切り替え、両アカウントが同じローカルリポジトリを操作するため切り替え時は`git pull`後に作業し作業単位ごとに`commit`・`push`する、というルール）。

**2026年8月17日、メインアカウントのPro契約が終了し、この2アカウント体制も終了した。** Claude Code利用の再開時期は未定（数週間〜数ヶ月の見込みだが確定していない）。再開後の運用体制（単一アカウントに戻すか等）は再開時に判断する。中断直前の状況・次回最初に確認すべき項目は`docs/backlog.md`「運用メモ」を参照。
...
| 26    | プロフィール編集機能（表示名・アバター画像。設定画面に追加、DBスキーマ変更なし） | 完了 | [phase-26](docs/phases/phase-26-profile-editing.md) |
| 27    | スクロールバーの見た目調整（`app/globals.css`のみ、ブランドトーンに統一） | 完了 | [phase-27](docs/phases/phase-27-scrollbar-styling.md) |
| 28    | 一時チャットの名前付け（作成時のみ。`rooms.name`を流用、DBマイグレーションあり） | 完了 | [phase-28](docs/phases/phase-28-temp-chat-naming.md) |
| 29    | 既読機能（FR-25追加。DM常時ON・グループはオーナーがON/OFF切替可、既読件数表示。`mark_room_read` RPC＋DBマイグレーションあり） | 完了 | [phase-29](docs/phases/phase-29-read-receipts.md) |
| 30    | 一時チャットの作成後リネーム（DBマイグレーションなし、既存`rooms_update_owner`ポリシーで完結） | 完了 | [phase-30](docs/phases/phase-30-temp-chat-rename.md) |
| 31    | サイドバー会話一覧への既読状態反映（DBマイグレーションあり）。あわせてPhase 26〜30をコード/DBレベルで検証。Claude Code利用中断前の最終セッション | 完了 | [phase-31](docs/phases/phase-31-sidebar-read-receipts.md) |
| 32    | チャット側実機QAで発見・修正したDBバグ2件（`search_users`の重複行・`get_or_create_dm_room`の一時チャット誤取得）を`docs/schema.sql`へ反映。2アカウント運用終了の記述をCLAUDE.md/READMEに反映（ドキュメント同期のみ、アプリコード変更なし） | 完了 | [phase-32](docs/phases/phase-32-post-pause-docs-sync.md) |
