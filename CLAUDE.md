...
1. PC幅で起動時認証を有効にしたアカウントで`/home`を開き、認証入力フォームが左寄りでないこと、体感として画面の真ん中よりやや上に表示され、以前より自然な位置に見えることを確認する。スマホ幅、および`/chat/[roomId]`の各チャットゲートでも同様に、真ん中よりやや上の位置に表示されることを確認する（`/home`と`/chat/[roomId]`の相対的な縦位置のズレ自体は、ヘッダー常時表示という設計上わずかに残る想定）
