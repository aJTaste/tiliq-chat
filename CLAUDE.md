...
1. PC幅で起動時認証を有効にしたアカウントで`/home`を開き、認証入力フォームが画面中央に表示されること（左寄りでないこと）。スマホ幅でも従来通り中央表示のままであること。あわせて`/chat/[roomId]`の各チャットゲートと見比べ、垂直位置のズレが体感で気にならない程度まで縮まっていることを確認する（ヘッダー常時表示という設計上、わずかな差自体は残る想定）
...
- `/home`のゲートと`/chat/[roomId]`のゲートの垂直位置は、`flex-1`追加により差を縮めたが、`/home`のヘッダー常時表示という設計を維持する限り原理的にヘッダーの高さぶんの差は残る。完全に一致させるにはヘッダーをオーバーレイ化する等の設計変更が必要（設定画面への逃げ道という別の意図的機能に影響しうるため、実施するかはユーザー確認後に判断する）
