...
- **FR-4（グループチャット作成）：** Phase 19でM1（最小スコープ）を実装済み。グループ作成
  ...
  送受信（送信者名表示）に対応。ただし「人数上限なし」の要件に対しては、実装上は工学的な
  安全策として合計50人（招待49人まで）のソフトキャップを設けている点が要件からの意図的な
  乖離として残る。またメンバー追加・削除・退出・オーナー譲渡等の管理UIは未実装（M1後の課題）。
  詳細はCLAUDE.md「Phase 19 の実装内容・詳細」参照。
...
- チャット一覧画面（「一覧」／「検索」の排他タブ。「一覧」側はすべて・フレンド・ストレンジャー・一時チャット・グループのサブフィルタ、「検索」側がユーザー追加パネル）
- チャット画面
- 画像送信UI
...
※ 「検索」タブについて（Phase 5で判明、Phase 15追記、Phase 23で更新）：Phase 15時点では
ユーザー検索とフレンド申請の管理は同じ文脈の操作であるため独立した検索タブとしては実装せず、
ユーザー追加UI（下記）の開閉式パネルに統合していた。Phase 23のサイドバー再設計で「一覧」／
「検索」の排他タブに変更し、検索は独立タブとして実装済み（`components/home/SidebarNav.tsx`）。
会話一覧側のサブフィルタは「すべて・フレンド・ストレンジャー・一時チャット・グループ」の5つ。
...
※ 実装状況に関する注記（Phase 15追記）：実際の実装ではServer Actions（`app/actions/*.ts`）が
大半を占めており、これらはHTTPレスポンスを直接返すRoute Handlerとは異なりステータスコードを
持たないアーキテクチャ上の制約から、上記の統一形式ではなく独自の
`{ success: boolean, error?: string }` 形式を返している。上記の規約に実際に準拠しているのは
`app/api/cloudinary/sign/route.ts`（Route Handler）のみである。意図的なアーキテクチャ上の
判断として許容し、Server Actionsを統一形式に追従させる対応は行わない。
...
※ 実装状況に関する注記（Phase 15追記）：上記の単体・結合・E2E・非機能テストは、2026年8月
時点でいずれも自動テストとしては実装していない（テストフレームワーク自体を未導入。
`__tests__`ディレクトリ等も存在しない）。代わりに、各Phase完了時に`npx tsc --noEmit`・
`npx eslint .`・クリーンビルドによる静的検証を行い、機能面の動作確認は本人による手動QA
（Phaseごとに実機確認用チェックリストを用意）で担保する運用としている。開発体制
（コーディングはClaude、動作確認は本人が担当）とプロジェクト規模を踏まえた意図的な判断
であり、自動テスト基盤の導入は2026-08-12時点でも見送っている。
