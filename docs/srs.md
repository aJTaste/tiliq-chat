...
※ 実装状況に関する注記（Phase 8〜9で判明、Phase 15追記）：

- **FR-4（グループチャット作成）：** 未実装。DB・RLSはグループチャットを許容する設計にして
  あるが、`app/chat/[roomId]/page.tsx` や `get_conversation_list` 等のアプリ側実装は
  「DM相手1人」を前提としたままで、UI上もホーム画面にグループタブの見た目のみ用意している
  （無効化表示）。優先度は「必須」のまま据え置くが、着手するPhaseは未割り当て。
- **FR-14（チャット内検索）：** 基本の検索機能（フレンド／ストレンジャー一覧内でのフィルタ）
  自体はPhase 9で実装済み。ただし「オプションで同グループメンバーも追加可」の部分は、
  FR-4（グループチャット）が未実装のため対応できておらず、FR-4実装後にあわせて対応する。
...
- チャット一覧画面（フレンド・ストレンジャー・グループの3タブ＋ユーザー追加パネル）
- チャット画面
- 画像送信UI
...
※ 「検索」タブについて（Phase 5で判明、Phase 15追記）：ユーザー検索とフレンド申請の管理は
同じ文脈の操作であるため、独立した検索タブとしては実装せず、ユーザー追加UI（下記）の
開閉式パネルに統合した。会話一覧のタブ自体は「フレンド・ストレンジャー・グループ」の3つ。
