...
│   │   ├── ChatRoomOptionsMenu.tsx   # チャットオプションメニュー（鍵トグル・非表示一覧・メンバー一覧・一時チャットを閉じる）
...
│       ├── HomeTabs.tsx          # 「一覧」タブ本体：すべて/フレンド/ストレンジャー/グループのサブフィルタ・一時チャットバッジ
│       ├── AddUserPanel.tsx      # 「検索」タブ本体：ユーザー検索・フレンド申請・簡易ブロック
│       ├── CreateGroupPanel.tsx  # グループ作成モーダル
│       ├── CreateTempChatPanel.tsx # 一時チャット作成モーダル（有効期限選択）
