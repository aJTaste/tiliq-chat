...
- **既読機能（FR-25）：** `room_members.last_read_at`（自分の閲覧位置）を`mark_room_read` RPCで更新し、`room_members`のRealtime購読（`ChatRoom.tsx`の既存メッセージ購読チャンネルに相乗り）で相手の更新を即座に反映する。表示ロジックは「自分が送った直近の既読済みメッセージ1件にのみバッジを付ける」方式（`MessageBubble.tsx`の`readLabel`props）。DMは常時ON、グループは`rooms.read_receipts_enabled`（オーナーが`GroupMembersPanel.tsx`から切替）でON/OFF可能。Phase 31でサイドバー会話一覧（`HomeTabs.tsx`）にも同じロジックを反映済み（`get_conversation_list`の`last_message_read`・`get_group_conversation_list`の`last_message_read_count`。ポーリング無しの静的取得のため、一覧側はチャット画面のようなRealtime即時更新ではなく次回一覧取得時に反映される）
